...
#    '''
 ...
 #   st.markdown(html_temp,unsafe_allow_html=True)
    ...
    if 0=<result<=50:
      st.text("Air Quality is Good")

    if 51=<result<=100:
      st.text("Air Quality Satisfactory")

    if 101<result<=250:
      st.text("Air is Moderately Polluted")

    if 251<=result<=350:
      st.text("Air Quality is Poor")

    if 351<=result<=430:
      st.text("Air Quality is Very Poor")

    if result>=431:
      st.text("Air Pollution is Severe")
